...
class Laboratory(models.Model):
    ...
    currency_id = fields.Many2one('res.currency', 'Currency', default=lambda self: self.env.user.company_id
                                  .currency_id.id,
                                  required=True)
    patient = fields.Many2one('res.partner', 'Patient', domain="[('patient_seq', '!=', 'New')]", required=True)
    appointment_id = fields.Many2one('medical.appointment', string="Appointment",
                                     domain="[('patient_id', '=', patient_id)]")
    doctor_id = fields.Many2one('hr.employee', string="Physician",
    ...
    test_seq = fields.Char(string='Test ID', required=True,
                           copy=False,
                           readonly=True,
                           index=True,
    ...
    lab_test_charge = fields.Monetary('Lab Charges', help="Charge on the lab test")
...
class TestResult(models.Model):
    ...
    _rec_name = 'test_sub_id'

    test_sub_id = fields.Many2one('test.test')
    patient_id = fields.Many2one('res.partner', domain="[('patient_seq', '!=', 'New')]", required=True)
    doctor_id = fields.Many2one('hr.employee', string="Physician",
                                domain="[('is_doctor','=','doctor')]")
