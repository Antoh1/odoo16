...
class createpurchaseorder(models.TransientModel):
    ...
    def default_get(self, default_fields):
        ...
        data = self.env['employee.purchase.requisition'].browse(self._context.get('active_ids', []))
        update = []
        for record in data.requisition_order_ids:
            update.append((0, 0, {
                'product_id': record.product_id.id,
                'product_uom': record.product_id.uom_id.id,
                'order_id': record.requisition_product_id.id,
                'name': record.description,
                'product_qty': record.quantity,
                'price_unit': 1,
                'product_subtotal': record.quantity*1,
            }))
    ...
    def action_create_purchase_order(self):
        ...
        value = []
        rq = self.env['employee.purchase.requisition'].browse(self._context.get('active_id'))
        pricelist = self.partner_id.property_product_pricelist
        partner_pricelist = self.partner_id.property_product_pricelist
        sale_order_name = rq.name
        ...
        sale_order = self.env['employee.purchase.requisition'].browse(self._context.get('active_ids', []))
        ...
        sale_order.write({'state': 'purchase_order_created'})
