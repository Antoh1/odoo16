...
class Patient(models.Model):
    ...
    patient_id = fields.Integer('Patient ID NO', help="The patient id number here")
    ...
    age = fields.Integer('Age', help="The field will show the age "
                                     "of the patient")
    address = fields.Text('Place', help="The field will show the address of the patient")
    doc_id = fields.Many2one('hr.employee', 'Doctor',
                             domain="[('is_doctor','=','doctor')]",
                             help="The doctor attending to the patient.")
    ...
    speciality = fields.Many2one('hospital.specialization', 'Speciality',
                                 help="Field which shows the speciality chosen for the appointment")
    ...
    qr_code = fields.Binary("QR Code", attachment=False,
                            ...
                            help="The qr code generated by the details of patients")
    ...
    def action_appointment_sent(self):
        ...
        template_id = self.env.ref(
            'rt_appointment.appointment_approval_email_template').id
    ...
    def action_appointment_cancel(self):
        """Patients are confirm their appointments by sending mails to doctor"""
        template_id = self.env.ref(
            'rt_appointment.appointment_cancel_email_template').id
        self.env['mail.template'].browse(template_id).send_mail(self.id,
                                                                force_send=True)
        self.state = "cancel"
        return
