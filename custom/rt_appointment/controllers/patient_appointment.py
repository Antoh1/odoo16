...
#############################################################################
#
#    Ropetech Solutions Pvt. Ltd.
#
#    Copyright (C) 2023-TODAY Ropetech Solutions(<https://www.ropetech.co.ke>)
...
class ServiceRequest(http.Controller):
    ...
    def doctor_appointment(self):
        ...
        return request.render("rt_appointment.doctor_appointment_form",
                              values)
        ...
        if kw:
            ...
            if not doctors.image_128:
                return request.render(
                    'rt_appointment.doctor_slot', data)
            else:
    ...
    def submit_slots(self, **kw):
        ...
        if kw:
            ...
            return request.render(
                'rt_appointment.patient_form', data)
        else:
            return request.redirect('/')
    ...
    def submit_final(self, **kw):
        ...
        if kw:
            ...
            return request.render(
                'rt_appointment.appointment_thanks', vals)
        else:
            return request.redirect('/')
    ...
    def print_report(self, **kwargs):
        ...
        if patient_id:
            pdf, _ = request.env.ref(
                'rt_appointment.print_patient_card').sudo()._render_qweb_pdf(
                [patient_object.id])
